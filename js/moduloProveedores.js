$("#btnAgregarProveedor").on("click", function () {
    ...
    $.ajax({
      type: "POST",
      data:
        "crear_proveedor=1&tipo_persona=" +
        tipo +
        "&nombre=" +
        nombre +
        "&tipo_documento=" +
  ...
  function eliminarProveedor(idpersona) {
    $.ajax({
      type: "POST",
      data: "eliminar_proveedor=1&pro_id=" + idpersona,
      url: "modules/proveedores/proveedoresController.php",
      dataType: "json",
      success: function (data) {
        var resultado = data.resultado;
  ...
  function editarProveedor(idpersona) {
    parametros = {
      editar_proveedor: 1,
      pro_id: idpersona,
    };
    $.ajax({
      ...
      success: function (respuesta) {
        ...
        console.log(datos);
        if (datos.length > 0) {
          $('#idpersona').val(datos[0]['idpersona']);
  ...
  $("#btnConfirmEditarProveedor").on("click", function () {
    ...
    $.ajax({
      ...
      success: function (data) {
        var resultado = data.resultado;
        if (resultado == 1) {
          $("#fromEditarProveedor").modal("hide");
          $("body").removeClass("modal-open");
          $(".modal-backdrop").remove();
